...
//            auth.inMemoryAuthentication().withUser("admin").password(encoder().encode("admin")).roles("ADMIN");
        ...
        protected void configure(HttpSecurity http) throws Exception {
            http.antMatcher("/api/title/**").authorizeRequests().anyRequest().hasRole("ADMIN")
                // log in
                .and().formLogin()
                .loginPage("/login")
                .permitAll()
...
//        @Override
//        protected void configure(HttpSecurity http) throws Exception {
//            http.antMatcher("/api/**").authorizeRequests().and()
//                    .antMatcher("/api/title/**").authorizeRequests().anyRequest().hasRole("ADMIN")
//                // log in
//                .and().httpBasic()
//                // logout
//                .and().logout().logoutUrl("/admin_logout").logoutSuccessUrl("/protectedLinks").deleteCookies("JSESSIONID").and().exceptionHandling().accessDeniedPage("/403").and().csrf().disable();
